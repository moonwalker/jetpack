...
const reduceAsync = (items, key, value) => (cb) => {
  const result = {};
...
module.exports = (apiUrl, product) => {
  const payload = {
    query: `{ sitemap(product: "${product}", includeLocale: true, includeNotfound: true, includeExcluded: true) { sitemaps { marketObj { code defaultCurrency { code } } localeObjs { code codeAlias } localeRoutes } product { apiKeys } routeLocales { route, locales } canonicalLocales { locale, canonicalLocale } } }`
  };
